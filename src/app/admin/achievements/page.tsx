...
import { Achievement, AchievementCategory, ACHIEVEMENT_CATEGORIES } from "@/types/achievement";
...
const AchievementsForm = () => {
  ...
  useEffect(() => {
    if (achievementSelected) {
      reset({ title: achievementSelected.title, category: achievementSelected.category });
    } else {
    ...
    <div className="w-full flex flex-col items-center justify-center min-h-screen px-4 relative py-16 lg:py-18 xl:py-20">
      ...
      <section className="w-full font-satoshi relative rounded-xl border-zinc-700 shadow-lg shadow-zinc-900 border-3 overflow-hidden py-6 px-4 space-y-8 md:space-y-7 md:w-140 xl:w-240 xl:px-8 z-50 xl:py-14 xl:space-y-12 3xl:w-260">
        ...
        <figure className="absolute inset-0 w-full h-full z-20">
          <Image src={imgFaustiForm} alt="img-Fausti-form" className="w-full h-full z-20 object-cover object-center" />
        </figure>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-7 relative z-50 xl:space-y-10">
          ...
          <div className="relative flex items-center justify-center gap-4">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? "Enviando..." : achievementSelected ? "Actualizar" : "Subir Logro"}
            </button>
          </div>
        </form>
      </section>
      ...
      {/* Listado */}
      <section className="text-balance relative z-50 w-full mt-10 lg:mt-16">
        <h6 className="text-4xl text-center text-zinc-300 mb-12 lg:text-5xl lg:mb-12">
          Logros existentes
        </h6>

        {/* Grid: 1 col mobile → 3 col desde lg */}
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:gap-6 xl:gap-8">
          {[...achievements].reverse().map((achievement, index) => (
            <div
              key={achievement._id}
              className="bg-gradient-to-tr from-zinc-800 to-sky-950 border border-zinc-600 rounded-xl p-4 flex flex-col gap-4 relative z-50 lg:p-5"
            >
              {/* Header de la card */}
              ...
              <div className="flex flex-col gap-3 mt-auto">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-sky-600/80 hover:bg-sky-600 text-white py-1.5 rounded-md text-xs font-medium transition duration-200 lg:text-sm"
                  >
                    Editar
                  </button>
                  <button
        ...
        {achievements.length === 0 && (
          <p className="text-center text-zinc-400 py-8">No hay logros registrados aún</p>
        )}
      </section>
    </div>
  );
