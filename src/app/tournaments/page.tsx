...
import { getFidePlayer, getFideStats } from "@/lib/api"; // ✅ getTournaments ya NO viene de aquí
...
export const metadata: Metadata = {
  ...
  description: "Información sobre torneos, estadísticas FIDE y progreso de Faustino Oro",
};

export const revalidate = 3600;

// ✅ Acceso directo a MongoDB — igual que en notices/page.tsx
async function getTournaments() {
  await connectToDatabase();
  const tournaments = await TournamentModel.find().sort({ startDate: -1 }).lean();
  return JSON.parse(JSON.stringify(tournaments));
}

export default async function TournamentsPage() {
  const [tournaments, fidePlayer, fideStats] = await Promise.all([
    getTournaments(), // ✅ ahora usa la función local de arriba
    getFidePlayer(),
    getFideStats(),
  ]);
  ...
  return (
    <section className="w-full pb-32">
      <section className="w-full min-h-screen bg-zinc-800 flex flex-col pt-30 gap-12 lg:gap-0 lg:pt-0 lg:flex-row text-balance">
        ...
        <article className="pl-6 relative w-full h-[24vh] flex flex-col items-start justify-center text-start md:border-b border-zinc-700 md:w-1/2 md:h-screen md:pl-12 lg:pt-6">
          ...
          <p className="relative text-sm md:text-lg text-zinc-400 flex justify-center items-center max-w-175 my-6 3xl:text-xl">
            ...
            Sigue su calendario y mantente al día con su increíble recorrido competitivo.
          </p>
      ...
      {fidePlayer ? (
        ...
        <section className="w-full py-16 flex items-center justify-center bg-zinc-800/40">
          <p className="text-zinc-500 text-sm">
            Estadísticas FIDE no disponibles temporalmente.
          </p>
        </section>
      )}
