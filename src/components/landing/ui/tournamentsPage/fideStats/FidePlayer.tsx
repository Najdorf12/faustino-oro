...
export default function FidePlayer({
  ...
  return (
    <section className="w-full relative pt-12 flex flex-col md:pt-0 md:h-screen md:flex-row overflow-hidden">
      ...
      {/* CONTAINER 75% */}
      <div className="w-full flex flex-col items-center justify-between pt-6 px-3 md:w-[75%] md:px-6 md:pb-6 lg:pt-9 ">
    ...
    <>
      ...
      <ul className="relative z-50 flex justify-center gap-4 mt-12 text-zinc-600 font-medium w-full text-xl md:mt-0 lg:text-2xl xl:gap-20 ">
        <li className="flex flex-col items-center justify-center gap-2">
          <Image src={iconStandar} alt="icon-standar" className="w-8 lg:w-9" />
          <span className="border-9 rounded-full w-28 h-28 bg-zinc-800 md:w-32 md:h-32 flex justify-center items-center border-zinc-700 text-zinc-200 text-2xl md:text-4xl">
        ...
        <li className="flex flex-col items-center justify-center gap-2">
          <Image src={iconRapid} alt="icon-standar" className="w-8 lg:w-9" />
          <span className="border-9 rounded-full w-28 h-28 bg-zinc-800 md:w-32 md:h-32 flex justify-center items-center border-zinc-700 text-zinc-200 text-2xl md:text-4xl">
            {fide.rapid_rating}
          </span>
          <div className="flex gap-2 self-center flex-col justify-center items-center">
        ...
        <li className="flex flex-col items-center justify-center gap-2">
          <Image src={iconBlitz} alt="icon-standar" className="w-8 lg:w-9" />
          <span className="border-9 rounded-full w-28 h-28 bg-zinc-800 md:w-32 md:h-32 flex justify-center items-center border-zinc-700 text-zinc-200 text-2xl md:text-4xl">
            {fide.blitz_rating}
          </span>
          <div className="flex gap-2 self-center flex-col justify-center items-center">
      ...
      <div className="w-full flex flex-col mt-9 md:mt-0">
        <h6 className="text-zinc-600 text-xl font-medium">Títulos</h6>
        <ul className="flex flex-col gap-1 mt-2">
          {stats.titles.map((title, i) => (
            <li
              key={i}
              className="text-lg text-zinc-300 flex justify-between w-full md:w-90"
            >
              {title.name}
      ...
      <ul className="flex flex-col lg:flex-row w-full gap-4 rounded-lg font-medium mt-12 md:mt-0">
        <div className="text-lg md:text-xl py-3 px-4 text-zinc-300 rounded-lg border border-sky-600 bg-linear-to-tl from-zinc-800 to-sky-700 w-full relative">
          <Image
            src={icon2}
            alt="icon-4"
        ...
        <div className="text-lg md:text-xl py-3 px-4 text-zinc-300 border border-sky-600 bg-linear-to-tl from-zinc-800 to-sky-700 w-full rounded-lg relative">
          <Image
            src={icon2}
            alt="icon-4"
            className="w-8 absolute top-3.5 right-1"
